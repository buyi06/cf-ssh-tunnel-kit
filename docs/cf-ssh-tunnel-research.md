...
Tunnel、DNS CNAME、SSH ingress、专用凭据和本机服务可由脚本可靠自动完成。该主机名会对 Internet 公开可达，连接安全由服务器 `sshd` 的密钥或密码验证把关。[8] 脚本对 `sshd` 的默认边界是只读：用 `sshd -T` 的有效配置判断密码登录是否可用，需要放行时打印 drop-in 命令交由用户执行。只有在用户明确同意（交互确认或 `--allow-password`）时才写入配置，且用可回滚的方式完成——优先写 `sshd_config.d` 下的独立文件而不动主配置；老系统改为在 `sshd_config` 顶部插入并留下 `.cf-ssh-tunnel.bak` 备份；写入后依次做 `sshd -t` 语法校验、重载、`sshd -T` 生效复核，任一步不通过立即回滚，避免把用户锁在机器外面。

登录可用性同样交给脚本自动收敛，避免出现"隧道通了但登不进去"的半成品状态：优先沿用既有公钥（只列出指纹、不碰密码）；只有在既无公钥、账户密码又为空或被锁时才生成 16 位随机密码，写入本机 `/etc/shadow` 并只在终端打印一次（脚本不保留明文副本、不写入任何文件）。已有密码的账户不会被静默重置——Linux 无法还原哈希对应明文，脚本只能说明情况并把重置决定权交给用户（`credentials --set-password`）。
