...
Tunnel、DNS CNAME、SSH ingress、专用凭据和本机服务可由脚本可靠自动完成。该主机名会对 Internet 公开可达，连接安全由服务器 `sshd` 的密钥或密码验证把关。[8] 脚本自身的边界是不读写 `sshd_config`：它只读取 `sshd -T` 的有效配置来判断密码登录是否可用，不做任何修改，需要放行时打印 drop-in 命令交由用户执行。
