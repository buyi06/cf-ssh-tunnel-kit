...
assert_contains "$help_output" 'credentials [--set-password|--allow-password]' '帮助文本包含登录信息命令'
assert_contains "$help_output" '交互运行时脚本会先询问' '安全说明交代放行需先确认'
...
assert_contains "$start_help" 'sudo bash start.sh [--mainland|--auto|--quic] [--allow-password] [--no-update]' '一键脚本帮助包含完整用法'
...
assert_contains "$script_text" 'allow_password_login' '可按需放行 sshd 密码登录'
assert_contains "$script_text" 'sshd -t -f "$config"' '写入 sshd 配置前做语法校验'
assert_contains "$script_text" 'revert_sshd_login_change' '校验失败或未生效时回滚'
assert_contains "$script_text" 'sshd_config.d/99-cf-ssh-tunnel.conf' '优先写入独立 drop-in 而非改主配置'
assert_contains "$script_text" 'cf-ssh-tunnel.bak' '放行前备份原 sshd 配置'
assert_contains "$script_text" 'confirm_allow_password' '交互运行时先征求同意'
assert_contains "$script_text" '--allow-password' '提供免询问的放行选项'
assert_contains "$script_text" "ALLOW_PASSWORD == 1" '仅在明确允许时自动放行'
